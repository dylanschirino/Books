...
books.id=book_id WHERE books.id = :id';
    ...
    public function getAuthorsByEditorId($id){
        $sql = ' SELECT DISTINCT authors.* FROM authors JOIN author_book ON authors.id=author_book.author_id JOIN books ON
books.id=book_id JOIN editors ON books.editor_id=editors.id WHERE editors.id = :id';
        $pdoSt = $this->connection->prepare($sql);
        $pdoSt->execute(['id'=>$id]);
        return $pdoSt->fetchAll();
    }
